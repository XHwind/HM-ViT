...
def visualize_single_sample_output_bev(pred_box, gt_box, pcd, dataset,
    ...
    ratio : float
        Resolution for BEV feature map
    ...
    # pcd = [] # not plot lidar
    # (img_row, img_col)
    bev_map = dataset.project_points_to_bev_map(pcd, ratio=ratio,
                                                cav_lidar_range=[L1, W1, H1,
                                                                 L2, W2, H2])
    # (img_row, img_col, 3)
    bev_map = \
        np.repeat(bev_map[:, :, np.newaxis], 3, axis=-1).astype(np.float32)
    bev_map = (bev_map * 255)
    # TODO enhancing visualization via overlaying effect
    # 1 means no overlaying effect. The intent is to make the plotted green and red boxes both visible. But it still needs more testing
    alpha = 1
    if gt_box is not None and len(gt_box):
        for i in range(gt_box.shape[0]):
            ...
            bbx = bbx[:, ::-1]
            bev_map_overlay = bev_map.copy()
            cv2.polylines(bev_map_overlay, [bbx], True, (0, 255, 0), 2)
            # bev_map = bev_map_overlay
            bev_map = bev_map_overlay * alpha + (1 - alpha) * bev_map
    if pred_box is not None:
        ...
        for i in range(num_bbx):
            ...
            bbx = bbx[:, ::-1]
            bev_map_overlay = bev_map.copy()
            cv2.polylines(bev_map_overlay, [bbx], True, (255, 0, 0), 2)
            bev_map = bev_map_overlay * alpha + (1 - alpha) * bev_map

    bev_map = np.transpose(bev_map, (1, 0, 2))
